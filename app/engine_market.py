"""Marketplace part of the engine."""
...
class MarketMixin:
    async def marketplace_pass(self) -> int:
        ...
        # Credit a fulfilled listing's card once so the offer is not recreated.
        ...
        if changed:
            self.db.set_json("fulfilled_seen", list(seen)[-1000:])
        ...
        if n_needed > 0:
            ...
            all_missing = self.db.missing_cards_grouped()
            ...
            # The server commits one copy of the offered card type.
            for pl in plans:
                try:
    ...
    async def _fulfill_useful(self, cfg: dict) -> int:
        ...
        my_missing = self.db.missing_card_ids()
