"""Tests de la couche SQLite (fichier temporaire)."""
import os
import tempfile
import time
import unittest
...
class Inventory(DbCase):
    def test_bump_new_then_dup(self):
        self.assertTrue(self.db.bump_inventory("s", "c1", "R"))    # nouvelle
        self.assertFalse(self.db.bump_inventory("s", "c1", "R"))   # doublon
        self.assertTrue(self.db.owns_card("c1"))
        self.assertFalse(self.db.owns_card("inconnue"))

    def test_decrement_floor_zero(self):
        self.db.bump_inventory("s", "c1", "R")
        self.db.decrement_inventory("s", "c1", by=5)
        self.assertFalse(self.db.owns_card("c1"))   # quantity tombée à 0

    def test_all_duplicates(self):
        self.db.bump_inventory("s", "c1", "R")
        self.db.bump_inventory("s", "c1", "R")      # quantity 2 -> doublon
        self.db.bump_inventory("s", "c2", "R")      # quantity 1 -> pas doublon
...
class RecycleFailures(DbCase):
    def test_mark_skips_summary_clear(self):
        now = time.time()
        r1 = self.db.mark_recycle_failure("p1", "wiki-1", "SR", 1800, now)   # 1er échec -> +1800
        self.db.mark_recycle_failure("p2", "wiki-2", "LR", 1800, now)
        r2 = self.db.mark_recycle_failure("p1", "wiki-1", "SR", 1800, now)   # 2e échec -> +3600
        self.assertAlmostEqual(r1, now + 1800, delta=2)
        self.assertAlmostEqual(r2, now + 3600, delta=2)                      # backoff croissant
...
class Progress(DbCase):
    def test_progress_view_owned_and_missing(self):
        self.db.upsert_series("s", "Série S", "#000", "#fff", 3)   # set_size 3
        self.db.bump_inventory("s", "c1", "R")
