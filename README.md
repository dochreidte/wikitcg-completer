...
- **Cookies live in the OS keyring**, never in the repo and never in a config file. A `WIKITCG_SESSION_<ACCOUNT_ID>` environment variable can supply one instead — it takes precedence over the keyring — and the dashboard flags it on startup if no keyring backend is available.
...
194 tests, standard library only — no test dependencies and no network:

```bash
python -m unittest discover -s tests -t .
```

They cover the engine loops, recycling policy, marketplace planning, account rotation, throttle and
retry logic, the localhost guard, the keyring wrapper and the HTTP API — all against fakes.
